                ...
                */
                ...
                if (segment) {
                    ...
                    if (isDonut || hStyle == '2d') {
                       ...
                       pie.Explode(segment[5], 25);

                        setTimeout(function () {obj.Set('chart.exploded', []);}, 500);
...
var angle = ((this.angles[i][1] - this.angles[i][0]) / 2) + this.angles[i][0];
    ...
    RGraph.Pie.prototype.Explode = function (index, size)
    {
        ...
        for (var i=0; i<size; ++i) {
            setTimeout(
                function ()
                {
                    __rgraph_pie_exploded__ = obj.Get('chart.exploded');
                    var value = typeof(__rgraph_pie_exploded__[index]) == 'number' ? __rgraph_pie_exploded__[index] : 0;
                    __rgraph_pie_exploded__[index] = value + 1;
                    obj.Set('chart.exploded', __rgraph_pie_exploded__);
    ...
    */
    RGraph.Pie.prototype.highlight_segment = function (segment)
    {
        ...
        context.moveTo(segment[0], segment[1]);
        context.arc(segment[0], segment[1], segment[2], RGraph.degrees2Radians(this.angles[segment[5]][0]), RGraph.degrees2Radians(this.angles[segment[5]][1]), 0);
